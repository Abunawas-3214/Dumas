...
<div class="main__content categories ticket-create">
    <div class="table-wrap">
        ...
        <h3 style="font-size: 1.3rem; margin-top: 10px">
        ...
        <form method="post"
            ...
            action="admin_submit_ticket.php" name="form1" id="submit-ticket" enctype="multipart/form-data">
            ...
            <?php if ($number_of_categories > 1): ?>
                <div class="form-group" style="margin-bottom: 0px;">
                    ...
                    &nbsp;
                    <button type="button" class="btn btn--blue-border change_category" name="cc-btn" id="cc-btn"
                        title="<?php echo $hesklang['chg_cat']; ?>"><?php echo hesk_getCategoryName($category); ?>
                        &nbsp;
            ...
            <div class="form-group">
                ...
                <input type="text" id="create_name" name="name"
                    class="form-control <?php if (in_array('name', $_SESSION['iserror'])) {
                        echo 'isError';
                    } ?>"
                    maxlength="50"
            ...
            <div class="form-group">
                ...
                <input type="<?php echo ($hesk_settings['multi_eml'] ? 'text' : 'email'); ?>"
                    class="form-control <?php if (in_array('email', $_SESSION['iserror'])) {
                        echo 'isError';
                    ...
                    <?php if ($hesk_settings['detect_typos']) {
                        echo ' onblur="Javascript:hesk_suggestEmail(\'email\', \'email_suggestions\', 1, 1)"';
                    } ?>>
            </div>
            <div id="email_suggestions"></div>
            <div class="form-group">
                <label
                    class="priority <?php if (in_array('priority', $_SESSION['iserror'])) {
                        echo 'isErrorStr';
                    } ?>"><?php echo $hesklang['priority']; ?>: <?php if ($hesk_settings['select_pri']) {
    ...
    // -->
                    ...
                    function setMessage(msgid) {
                        ...
                        if (document.getElementById) {
                            if (document.getElementById('moderep').checked) {
                                ...
                                <?php else: ?>
                                    document.getElementById('HeskMsg').innerHTML = '<textarea style="height: inherit"
                                    class="form-control" name = "message" id = "message" rows = "12" cols = "60" > '+myMsg+'</textarea > ';
                                    <?php endif; ?>
                                        document.getElementById('HeskSub').innerHTML='<input class="form-control" type="text" name="subject"
                                id = "subject" maxlength = "70" value = "'+mySubject+'" > ';
                            }
                            else {
                <?php if ($hesk_settings['staff_ticket_formatting'] == 2): ?>
                ...
                <?php else: ?>
                    var oldMsg = escapeHtml(document.getElementById('message').value);
                                    document.getElementById('HeskMsg').innerHTML = '<textarea style="height: inherit" class="form-control"
                                    name = "message" id = "message" rows = "12" cols = "60" > '+oldMsg+myMsg+'</textarea > ';
                                    <?php endif; ?>
            ...
            if (strlen($can_options)) {
                ...
                <div class="form-group">
                    <label>
                        <?php echo $hesklang['select_ticket_tpl']; ?>:
                    </label>
                    <div class="dropdown-select center out-close">
                        <select name="saved_replies" onchange="setMessage(this.value)">
                            <option value="0"> -
                                <?php echo $hesklang['select_empty']; ?> -
                            </option>
            ...
            elseif (hesk_checkPermission('can_man_ticket_tpl', 0)) {
                ?>
                <div class="form-group">
                    <label><a href="manage_ticket_templates.php" class="link">
                            <?php echo $hesklang['ticket_tpl_man']; ?>
                        </a></label>
                </div>
                <?php
            }
            ...
            <div class="form-group">
                <label>
                    <?php echo $hesklang['subject'] . ': ' . ($hesk_settings['require_subject'] == 1 ? '<span class="important">*</span>' : ''); ?>
                </label>
                <span id="HeskSub"><input
                        class="form-control <?php if (in_array('subject', $_SESSION['iserror'])) {
                            echo 'isError';
                        } ?>"
                        type="text" name="subject" id="subject" maxlength="70"
                        value="<?php if (isset($_SESSION['as_subject'])) {
                            echo stripslashes(hesk_input($_SESSION['as_subject']));
                        } ?>"></span>
            </div>
            <div class="form-group">
                <label>
                    <?php echo $hesklang['message'] . ': ' . ($hesk_settings['require_message'] == 1 ? '<span class="important">*</span>' : ''); ?>
                </label>
                <span id="HeskMsg">
                    <textarea style="height: inherit"
                        class="form-control <?php if (in_array('message', $_SESSION['iserror'])) {
                            echo 'isError';
                        } ?>"
                        name="message" id="message" rows="12"
                        cols="60"><?php if (isset($_SESSION['as_message'])) {
                ...
                <div class="checkbox-list">
                    ...
                    <?php if (hesk_checkPermission('can_view_tickets', 0)): ?>
                        <div class="checkbox-custom">
                            ...
                            <label for="create_show1">
                                <?php echo $hesklang['otas']; ?>
                            </label>
                        </div>
                    <?php endif; ?>
                </div>
                    ...
                    <div class="calendar--button">
                        ...
                        <input name="due_date" id="due_date"
                            value="<?php if (isset($due_date)) {
                                echo stripslashes(hesk_input($due_date));
                            } ?>" type="text"
                            class="datepicker">
                    </div>
                    <div class="calendar--value" style="<?php echo empty($due_date) ? '' : 'display: block'; ?>">
                        <span>
                            <?php echo isset($due_date) ? stripslashes($due_date) : ''; ?>
                        </span>
                        <i class="close">
                            <svg class="icon icon-close">
            ...
            <?php if ($hesk_settings['can_sel_lang']): ?>
                <div class="form-group">
                    <label for="as_language">
                        <?php echo $hesklang['tlan']; ?>:
                    </label>
            ...
            <?php if (hesk_checkPermission('can_assign_others', 0)) { ?>
                <div class="form-group">
                    <label>
                        <?php echo $hesklang['asst2']; ?>:
                    </label>
                    <select name="owner" id="owner-select" <?php if (in_array('owner', $_SESSION['iserror'])) {
                        echo ' class="isError" ';
                    } ?>>
                        <option value="-1"> &gt;
                            <?php echo $hesklang['unas']; ?> &lt;
                        </option>
                        <?php
                ...
                <div class="form-group">
                    ...
                    <div class="checkbox-custom">
                        ...
                        <label for="create_assing_to_self1">
                            <?php echo $hesklang['asss2']; ?>
                        </label>
                    </div>
                </div>
                <?php
        ...
        if ($number_of_categories > $hesk_settings['cat_show_select']) {
            ...
            <div class="table-wrap">
                ...
                <form action="new_ticket.php" method="get" class="form">
                    ...
                    <button style="margin-top: 10px" type="submit" class="btn btn-full">
                        <?php echo $hesklang['c2c']; ?>
                    </button>
                </form>
                <script>
        ...
        // Otherwise print quick links
            ...
            <div class="nav">
                <?php foreach ($hesk_settings['categories'] as $k => $v): ?>
                    <a href="new_ticket.php?a=add&amp;category=<?php echo $k; ?>"
                        class="navlink <?php if ($number_of_categories > 8)
                        ...
                        <div>
                            <h5 class="navlink__title"><!--[if IE]> &raquo; <![endif]-->
                                <?php echo $v['name']; ?>
                            </h5>
                        </div>
                    </a>
